...
svm =  pickle.load(open('models/rf.pkl','rb'))
...
def predict():
    mylist = list(request.form.values())

    feature = vec.transform(mylist)
  
    output = svm.predict(feature) > 0.5  # features Must be in the form [[a, b]]
